...
const PlusMinusReport = () => {
  ...
  console.log(ledgerData, "ledger data plus minus report");
  ...
  React.useEffect(() => {
      betService
        ...
        .then((res: AxiosResponse<{ data: any }>) => {
            console.log(res, "one ledger data plus minus report");
            const fresh = res?.data?.data[0]
            const filterfresh = fresh.filter((m: any) => m.matchId == 34912508);
            console.log(filterfresh, "filtered fresh data plus minus report");
