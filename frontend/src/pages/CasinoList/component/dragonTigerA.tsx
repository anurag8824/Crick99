...
const DragonTigerA = (props: any) => {
  ...
  const singleCard = (MarketName: any, marketIndex: any) => {
    ...
    return (
      <>
        <div style={{display:"none"}} className='col-lg-12 col-12'>
          <h4 className='m-b-0 text-center text-uppercase'>
            {MarketName} &nbsp;
            {`12.00`}
          </h4>
        </div>
        <div style={{display:"none"}} className='card-dt col-lg-12  col-12 text-center m-t-10 card-seven'>
  ...
  const buttonLayout = (classData: string, marketIndex: any) => {
    console.log("finalMarketList", liveMatchData)

    const finalMarketList = liveMatchData?.defaultMarkets?.filter((ItemN: any, index: number) => marketIndex.indexOf(index) > -1 && ItemN?.MarketName !== "Pair") || []
    return (
      finalMarketList.map((Item: any, key: number) => {
        const market = Item?.Runners?.[0] || {}
